...
namespace Assets.Scripts.TouchKit
{
    /// <summary>
    ...
    public partial class TouchKit
    {
        ...
        /// <summary>
        ///     returns true if mouse input should be processed as touch input. it will be true when the Unity Remote is not
        ///     active.
        /// </summary>
        private bool shouldProcessMouseInput()
        {
            ...
            if (simulateMultitouch &&
                (_hasActiveSimulatedMultitouch || Input.GetKey(KeyCode.LeftAlt) || Input.GetKeyUp(KeyCode.LeftAlt)))
            {
                ...
                else if (Input.GetKey(KeyCode.LeftShift))
                {
                    ...
                    var lastMousePosition = _simulatedMultitouchStartPosition.Value +
                                            (_simulatedMultitouchStartPosition.Value - _simulatedMousePosition);
                ...
                {
                    var diff = new Vector3(Input.mousePosition.x - _simulatedMultitouchStartPosition.Value.x,
                        Input.mousePosition.y - _simulatedMultitouchStartPosition.Value.y);
        ...
        {
            if (Scripts.TouchKit.TouchKit._instance == null)
                return;
            ...
            if (drawTouches)
            {
                ...
                if (_simulatedMultitouchStartPosition.HasValue && !_hasActiveSimulatedTouch)
                {
                    ...
                    var simulatedPos =
                        Camera.main.ScreenToWorldPoint(new Vector3(_simulatedMousePosition.x, _simulatedMousePosition.y,
                            Camera.main.farClipPlane));
